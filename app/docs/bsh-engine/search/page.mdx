...
The **BSH Search** object is a powerful query system that allows you to search, filter, group, sort, and paginate data across entities in the BSH Engine. It provides a flexible and type-safe way to construct complex queries.
...
<Tabs items={['Documentation', 'TypeScript', 'Examples', 'Best Practices']}>
    ...
    ### `query` [#query]
    The main filtering mechanism using query leaves to build complex conditions.
    ...
    <Tabs items={['Fields', 'Comparison Operators', 'Logical Operators']}>
        <Tabs.Tab title="Fields">
            ...
            | `query` | Nested query leaves for complex conditions | if used logical operator |
        </Tabs.Tab>
    ...
    <Tabs.Tab title="TypeScript">
        ...
        export type QueryLeaf<T = unknown> = {
    ...
    This query will search for users with email containing `@bsh.com` and return the first 10 results.
